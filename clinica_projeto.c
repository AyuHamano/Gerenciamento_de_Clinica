...
typedef struct {
   ...
   void imprime_horarios() {
    ...
    //MARCAR CONSULTA
    void Marcar_consulta(LISTA *l, int j){
        int m, d, h;
    ...
    void desmarcar_consulta (LISTA *l, int j) {
        int i, dia, mes, flag = 0;
        printf("Entre com o dia da consulta: \n");
        scanf("%d", &dia);
        printf("Entre com o mes da consulta: \n");
        scanf("%d", &mes);
        for(i=0; i<H; i++) {
            if((l->ag[dia][mes].atend[i].cod_paciente == l->pacientes[j].cod)&&(l->ag[dia][mes].atend[i].status = 'O')) {
                l->ag[dia][mes].atend[i].status = 'L';
                flag = 1;
            ...
            switch(op){
                ...
                case 2:
                    ...
                    if(j>=0) {
                        ...
                        printf("Paciente ainda não está cadastrado!\n");
					}
                    enter_para_retornar();
                  break;
                //desmarcar
